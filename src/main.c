...
"usage: ssat [ <option> ... ] [ <dimacs> [ <proof>] ]\n"
"\n"
"where '<option>' is one of the following\n"
"\n"
"  -h                   print this option summary\n"
"  --version            print this solver version and exit\n"
...
"and '<dimacs> is and optionally compressed CNF in DIMACS format by\n"
...
#include "ssat.h" // the main code is declared in this header
...
// Some static global data structures used in 'main.c'. We'll clean this up later.

/*------------------------------------------------------------------------*/

// Needed by DIMACS parser.
...
static long lineno = 1; // Line numer for parse error messages.
static uint64_t bytes;  // Read bytes for verbose message.

/*------------------------------------------------------------------------*/

// Static global solver and parsed number of varables.
...
// Note that global vbls in 'C' are initialized to zero and thus these options are all disabled initially.
static const char *ascii; // Force ASCII format for proof files.
static const char *binary; // Force binary format for proof files.
static const char *force; // Overwrite proofs and relax parsing.

#ifdef LOGGING
const char *logging;
#endif
static const char *quiet; // Turnoff default 'verbose' mode.
const char *no_witness;   // Do not print sat assignment.

static int verbose = 1;   // Verbose level (unless 'quiet' is set).

/*------------------------------------------------------------------------*/

// Store parsed XORs for delayed encoding of XPRs (particuklarly with '-f')
...
// Line buffer for pretty-printing witness ('v' lines following the SAT)
// competition output format fit to at most 78 chars per line).
...
// These declaration provide nice warnings messages if these functions have a format
// string which does not match the type of one of its args.
